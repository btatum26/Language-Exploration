...
- Geometry type is permitted by the referenced entry.
...
6. Optionally resolves entry definitions for display.
7. Returns one `AnnotatedRecordingSnapshot`.
...
History listing returns compact rows containing revision ID and number, timestamp, author, message, and annotation count. Detailed diffs are calculated only when explicitly requested.
